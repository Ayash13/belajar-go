...
func (p *kafkaPublisher) Publish(ctx context.Context, topic string, key string, payload interface{}) error {
	...
	span.SetAttributes(attribute.String("kafka.topic", topic), attribute.String("kafka.key", key))
	...
	if err := p.writer.WriteMessages(ctx, msg); err != nil {
		...
		config.Log.Error("Failed to publish Kafka event", zap.String("topic", topic), zap.Error(err))
		return err
	}

	span.SetStatus(codes.Ok, "published")
	config.Log.Info("Kafka event published", zap.String("topic", topic), zap.String("key", key))
	return nil
}
